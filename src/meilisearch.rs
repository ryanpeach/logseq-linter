...
mod tests {
    ...
    async fn test_add_documents_and_search() {
        dotenv().ok();
        let meilisearch_url = env::var("MEILISEARCH_URL").unwrap_or("http://localhost:7700".to_string());
        let meilisearch_api_key = env::var("MEILISEARCH_API_KEY").unwrap_or("masterKey".to_string());
        ...
        ], Some("id")).await.expect("Cannot add documents");
